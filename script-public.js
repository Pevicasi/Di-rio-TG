...
function pdfFileName() {
  const name = pdfSafeText(appData?.profile?.name || "TirzeTrack")
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-zA-Z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "") || "TirzeTrack";
  const date = new Date().toISOString().slice(0, 10);
  return `TirzeTrack-${name}-${date}.pdf`;
}

function addPdfHeader(pdf, title, subtitle = "") {
  ...
  pdf.setFontSize(9);
  if (subtitle) pdf.text(subtitle, 14, 19);
...
function addPdfFooter(pdf) {
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    pdf.text("Relatório gerado pelo TirzeTrack", 14, pageHeight - 7);
    pdf.text(`Página ${page} de ${pages}`, pageWidth - 14, pageHeight - 7, { align: "right" });
  }
}

function ensurePdfSpace(pdf, y, needed = 24) {
  const pageHeight = pdf.internal.pageSize.getHeight();
  if (y + needed <= pageHeight - 18) return y;
  pdf.addPage();
  addPdfHeader(pdf, appData?.title || "TirzeTrack", `Atualizado em ${appData?.updatedAt || "-"}`);
  return 36;
}
...
async function exportPdfReport() {
  ...
  if (!window.jspdf?.jsPDF) {
    if (status) {
      status.hidden = false;
      status.className = "import-status error";
      status.textContent = "Não foi possível carregar o gerador de PDF. Verifique a conexão e tente novamente.";
    }
    return;
  }
  ...
  if (status) {
    status.hidden = false;
    status.className = "import-status loading";
    status.textContent = "Preparando relatório completo...";
  }

  try {
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4", compress: true });
    const summary = weightSummary(appData);
    const profile = appData.profile || {};
    const goal = appData.goal || {};
    const treatment = appData.treatment || {};
    const pageWidth = pdf.internal.pageSize.getWidth();

    addPdfHeader(pdf, appData.title || "Acompanhamento com Tirzepatida", `${profile.name || ""} • Atualizado em ${appData.updatedAt || "-"}`);
    let y = 37;

    pdf.setFontSize(10);
    pdf.setFont("helvetica", "normal");
    pdf.text(`Paciente: ${profile.name || "-"}`, 14, y);
    pdf.text(`Idade: ${profile.age || "-"} anos`, 105, y);
    y += 6;
    pdf.text(`Altura: ${Number(profile.heightM || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} m`, 14, y);
    pdf.text(`Início do tratamento: ${treatment.startDate || "-"}`, 105, y);
    y += 10;

    y = addPdfSectionTitle(pdf, "Resumo geral", y);
    pdf.autoTable({
      startY: y,
      theme: "grid",
      head: [["Peso inicial", "Peso atual", "Perda acumulada", "Meta", "Falta"]],
      body: [[kg(summary.initial), kg(summary.current), kg(summary.loss), kg(goal.targetWeightKg), kg(Math.max(0, summary.current - Number(goal.targetWeightKg || 0))) ]],
    ...
    y = addPdfSectionTitle(pdf, "Tratamento", y);
    pdf.autoTable({
      startY: y,
      theme: "striped",
      body: [
        ["Medicamento", treatment.medication || "-"],
        ["Concentração", treatment.concentration || "-"],
        ["Dose semanal", treatment.weeklyDose || "-"],
        ["Data de início", treatment.startDate || "-"]
      ],
    ...
    if (chart && chart.width && chart.height) {
      y = addPdfSectionTitle(pdf, "Gráfico de evolução do peso", y);
      y = ensurePdfSpace(pdf, y, 72);
      const image = chart.toDataURL("image/png", 1.0);
      const maxWidth = pageWidth - 28;
      const height = Math.min(70, maxWidth * chart.height / chart.width);
      pdf.addImage(image, "PNG", 14, y, maxWidth, height, undefined, "FAST");
      y += height + 8;
    }

    y = addPdfSectionTitle(pdf, "Histórico de pesagens", y);
    pdf.autoTable({
      startY: y,
      theme: "grid",
      head: [["Data", "Peso", "Variação"]],
      body: (appData.weights || []).map((item, index, list) => {
        const previous = index ? Number(list[index - 1].valueKg) : Number(item.valueKg);
        const variation = Number(item.valueKg) - previous;
        return [item.date || "-", kg(item.valueKg), index ? `${variation > 0 ? "+" : ""}${variation.toFixed(2).replace(".", ",")} kg` : "Início"];
      }),
      styles: { font: "helvetica", fontSize: 9, cellPadding: 2.5 },
    ...
    y = addPdfSectionTitle(pdf, "Aplicações", y);
    pdf.autoTable({
      startY: y,
      theme: "striped",
      head: [["Nº", "Data", "Hora", "Dose", "Local"]],
      body: (appData.applications || []).map(item => [item.number ?? "-", item.date || "-", item.time || "-", item.dose || "-", pdfSafeText(item.location) || "-"]),
      styles: { font: "helvetica", fontSize: 8.5, cellPadding: 2.3, overflow: "linebreak" },
      headStyles: { fillColor: [15, 118, 110] },
      columnStyles: { 0: { cellWidth: 12 }, 1: { cellWidth: 25 }, 2: { cellWidth: 18 }, 3: { cellWidth: 20 } },
      margin: { left: 14, right: 14 }
    });
    y = pdf.lastAutoTable.finalY + 8;

    y = addPdfSectionTitle(pdf, "Resumo semanal", y);
    pdf.autoTable({
      startY: y,
      theme: "grid",
      head: [["Semana", "Período", "Informações"]],
      body: (appData.weeks || []).map(item => [item.title || "-", item.period || "-", (item.lines || []).map(pdfSafeText).filter(Boolean).join("\n")]),
      styles: { font: "helvetica", fontSize: 8.2, cellPadding: 2.5, overflow: "linebreak", valign: "top" },
      headStyles: { fillColor: [15, 118, 110] },
      columnStyles: { 0: { cellWidth: 25 }, 1: { cellWidth: 34 } },
      margin: { left: 14, right: 14 }
    });
    y = pdf.lastAutoTable.finalY + 8;

    y = addPdfSectionTitle(pdf, "Diário", y);
    pdf.autoTable({
      startY: y,
      theme: "striped",
      head: [["Data", "Alimentação", "Fome", "Efeitos", "Observações"]],
      body: (appData.diary || []).map(item => [item.date || "-", pdfSafeText(item.meals), pdfSafeText(item.hunger), pdfSafeText(item.effects), pdfSafeText(item.notes)]),
      styles: { font: "helvetica", fontSize: 7.5, cellPadding: 2, overflow: "linebreak", valign: "top" },
      headStyles: { fillColor: [15, 118, 110] },
      columnStyles: { 0: { cellWidth: 22 }, 1: { cellWidth: 43 }, 2: { cellWidth: 35 }, 3: { cellWidth: 35 }, 4: { cellWidth: 47 } },
      margin: { left: 14, right: 14 }
    });
    y = pdf.lastAutoTable.finalY + 8;

    y = addPdfSectionTitle(pdf, "Observação geral", y);
    y = ensurePdfSpace(pdf, y, 28);
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(9);
    const observation = pdf.splitTextToSize(pdfSafeText(appData.generalObservation) || "Sem observação geral.", pageWidth - 28);
    pdf.text(observation, 14, y);
    y += observation.length * 4.5 + 5;
    const notice = pdf.splitTextToSize(pdfSafeText(appData.medicalNotice), pageWidth - 28);
    pdf.setFontSize(8);
    pdf.setTextColor(100, 116, 139);
    pdf.text(notice, 14, y);
    ...
    if (status) {
      status.className = "import-status error";
      status.textContent = `Não foi possível gerar o PDF: ${error.message}`;
    }
  } finally {
